...
/// <summary>
/// Utility class for parsing Minecraft-style color and formatting codes on the server side.
/// </summary>
internal static class ColorCodeParser {
    ...
    private static readonly Dictionary<char, string> AnsiCodes = new() {
        // Colors
        { '0', "\x1b[97m" }, // Black (note: mapping follows existing implementation)
        { '1', "\x1b[34m" }, // Dark Blue
        ...
        { 'e', "\x1b[93m" }, // Yellow
        { 'f', "\x1b[30m" }, // White (note: mapping follows existing implementation)
    ...
    public static string ParseToAnsi(string message) {
        ...
        for (int i = 0; i < message.Length; i++) {
            if (message[i] == '&') {
                ...
                // Lone '&' or unknown sequence -> keep as-is
                result.Append('&');
                continue;
            }
            ...
            if (message[i] == '&') {
                ...
                if (i + 1 < message.Length) {
                    ...
                    if (AnsiCodes.ContainsKey(code)) {
                        i++; // Skip the code character (strip it)
                        continue;
                    }
                }
                // Lone '&' or unknown sequence -> keep as-is
