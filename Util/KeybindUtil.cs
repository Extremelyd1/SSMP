...
public static class KeybindUtil {
    ...
    /// <returns>The <c>KeyOrMouseBinding</c> instance.</returns>
    public static InputHandler.KeyOrMouseBinding GetKeyOrMouseBinding(this PlayerAction action) {
        foreach (var src in action.Bindings) {
            var ret = src switch {
                KeyBindingSource { Control.IncludeCount: 1 } kbs => new InputHandler.KeyOrMouseBinding(
    ...
    /// <returns>A nullable <c>KeyOrMouseBinding</c> instance.</returns>
    public static InputHandler.KeyOrMouseBinding? ParseBinding(string src) {
        ...
        if (Enum.TryParse<Mouse>(src, out var mouse)) {
    ...
    /// <returns>The control type for the input corresponding to the action.</returns>
