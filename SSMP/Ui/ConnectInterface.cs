...
internal class ConnectInterface {
    ...
    private IEnumerator JoinLobbyCoroutine(string lobbyId, string username) {
        ...
        if (lobbyType == PublicLobbyType.Steam) {
            ConnectToSteamLobby(connectionData, username, holePunchSocket);
        } else {
    ...
    /// </summary>
    private void ConnectToSteamLobby(string connectionData, string username, Socket? holePunchSocket) {
        // Steam doesn't need hole-punch socket
        CleanupHolePunchSocket(holePunchSocket);
