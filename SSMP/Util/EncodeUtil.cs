...
public static class EncodeUtil {
    ...
    public static object? DecodeSaveDataValue(string? name, byte[] encodedValue) {
        ...
        if (type == "SSMP.Serialization.BossSequenceDoorCompletion") {
        ...
        if (type == "SSMP.Serialization.BossStatueCompletion") {
        ...
        if (type == "System.Collections.Generic.List`1[SSMP.Math.Vector3]") {
            var length = BitConverter.ToUInt16(encodedValue, 0);
        ...
        if (type == "SSMP.Serialization.MapZone") {
            if (encodedValue.Length != 1) {
