...
internal class SteamEncryptedTransport : IReliableTransport
{
    ...
    private void Receive(byte[]? buffer, int offset, int length)
    {
        ...
        if (!SteamNetworking.IsP2PPacketAvailable(out var packetSize, P2P_CHANNEL)) return;

        if (!SteamNetworking.ReadP2PPacket(_receiveBuffer, MAX_PACKET_SIZE, out packetSize, out CSteamID remoteSteamId, P2P_CHANNEL)) return;
